...
// ─── Types (identiques à la version précédente) ────────────────────────────────
...
const VerticalBar: React.FC<{ value: number; color: string }> = ({ value, color }) => {
  const heightPercent = Math.max(value * 1.2, 4); // pour éviter les barres trop petites
  return (
...
const EnseignantDashboard: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-950 flex">
      {/* Sidebar identique à la version précédente, conservé pour la navigation */}
      <aside className="hidden lg:flex flex-col w-64 h-screen sticky top-0 bg-gray-900/30 border-r border-gray-800/50 p-6">
        ...
        <nav className="space-y-1 mb-6">
          ...
          <Link to="/enseignant/dashboard" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-emerald-500/10 text-emerald-400 font-medium text-sm">
          ...
          <Link to="/enseignant/quiz/create" className="flex items-center gap-3 px-4 py-3 rounded-xl text-gray-400 hover:text-white hover:bg-gray-800/50 font-medium text-sm">
  <BookOpen size={18} /> Évaluations
</Link>
  ...
  <BookOpen size={18} /> progression
      ...
      <main className="flex-1 p-6 lg:p-10 overflow-y-auto">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              <input
                ...
                className="w-64 bg-gray-800/50 border border-gray-700 rounded-xl pl-10 pr-4 py-2.5 text-white placeholder-gray-500 outline-none focus:border-emerald-500 text-sm"
              />
            </div>
           <NotificationBell />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          <Link to="/enseignant/cours/create" className="group bg-gray-900/50 border border-gray-800 rounded-2xl p-5 hover:border-emerald-500/30 transition-all flex items-center gap-4">
            ...
            <div>
              <h3 className="text-white font-semibold">Créer un module</h3>
              <p className="text-gray-500 text-sm">Définissez un nouveau cours</p>
            </div>
            <ChevronRight size={20} className="text-gray-500 ml-auto" />
          </Link>
          <Link to="/enseignant/cours/create" className="group bg-gray-900/50 border border-gray-800 rounded-2xl p-5 hover:border-blue-500/30 transition-all flex items-center gap-4">
            <div className="w-12 h-12 bg-blue-500/20 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
              <FileText size={24} className="text-blue-400" />
            </div>
            <div>
              <h3 className="text-white font-semibold">Déposer une ressource</h3>
              <p className="text-gray-500 text-sm">PDF, vidéo ou code source</p>
            </div>
            <ChevronRight size={20} className="text-gray-500 ml-auto" />
          </Link>
          <Link to="/annonces" className="group bg-gray-900/50 border border-gray-800 rounded-2xl p-5 hover:border-purple-500/30 transition-all flex items-center gap-4">
            <div className="w-12 h-12 bg-purple-500/20 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
              <Megaphone size={24} className="text-purple-400" />
            </div>
            <div>
              <h3 className="text-white font-semibold">Publier une annonce</h3>
              <p className="text-gray-500 text-sm">Communiquez avec votre promotion</p>
            </div>
            <ChevronRight size={20} className="text-gray-500 ml-auto" />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-10">
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col items-center">
            <CircularProgress value={dashboard?.progression_globale || 0} color="#10b981" label="Progression moyenne" />
            <p className="text-xs text-gray-500 mt-3 text-center">60% leçons + 40% quiz</p>
          </div>
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col items-center">
            <CircularProgress value={Math.round((dashboard?.nb_etudiants_suivis || 0) / 10 * 100)} color="#3b82f6" label="Étudiants suivis" />
            <p className="text-lg font-bold text-white mt-2">{dashboard?.nb_etudiants_suivis}</p>
          </div>
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col items-center">
            <CircularProgress value={Math.min((dashboard?.lecons_consultees || 0) / 10, 100)} color="#8b5cf6" label="Leçons consultées" />
            <p className="text-lg font-bold text-white mt-2">{dashboard?.lecons_consultees}</p>
          </div>
          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6 flex flex-col items-center">
            <CircularProgress value={(dashboard?.nb_etudiants_risque || 0) * 10} color="#ef4444" label="Étudiants à risque" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          <div className="lg:col-span-2 bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
            <h3 className="text-white font-semibold mb-6 flex items-center gap-2">
              <BarChart3 size={20} className="text-emerald-400" /> Progression par module
            </h3>
            ...
            <p className="text-gray-500 text-xs mt-4">
              60% leçons consultées + 40% moyenne aux quiz — {dashboard?.total_tentatives} tentatives enregistrées
            </p>
          </div>

          <div className="bg-gray-900/50 border border-gray-800 rounded-2xl p-6">
            <h3 className="text-white font-semibold mb-4 flex items-center gap-2">
              <Target size={20} className="text-amber-400" /> Top étudiants
            </h3>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="h-1.5 w-full bg-gray-800 rounded-full mt-1">
                        <div className="h-1.5 bg-emerald-400 rounded-full" style={{ width: `${etu.progression}%` }}></div>
                      </div>
                    </div>
                  </div>
                  <span className="text-emerald-400 font-semibold text-sm">{etu.progression}%</span>
                </div>
              ))}
            </div>
          </div>
            ...
            {dashboard?.modules.map((mod) => (
              <div key={mod.module_id} className="bg-gray-900/50 border border-gray-800 rounded-2xl p-5 hover:border-gray-700 transition-all">
                <div className="flex items-start justify-between mb-2">
                  <div>
                    <p className="text-sm font-bold text-white">{mod.titre}</p>
                    <p className="text-xs text-gray-500">{mod.code} • S{mod.semestre}</p>
                  </div>
                  {mod.nb_a_risque > 0 && (
                    <span className="text-xs px-2 py-0.5 bg-red-500/20 text-red-400 rounded-full">
                      {mod.nb_a_risque} à risque
                    </span>
                  )}
                </div>
                <div className="space-y-3 mt-4">
                  {/* Barre progression */}
                  <div>
                    <div className="flex justify-between text-xs text-gray-400 mb-1"><span>Progression</span><span>{mod.progression_moyenne}%</span></div>
                    <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${mod.progression_moyenne}%` }}></div>
                    </div>
                  </div>
                  {/* Barre couverture leçons */}
                  <div>
                    <div className="flex justify-between text-xs text-gray-400 mb-1"><span>Couverture leçons</span><span>{mod.couverture_lecons}%</span></div>
                    <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${mod.couverture_lecons}%` }}></div>
                    </div>
                  </div>
                  {/* Barre moyenne quiz */}
                  <div>
                    <div className="flex justify-between text-xs text-gray-400 mb-1"><span>Moy. quiz</span><span>{mod.moyenne_quiz}/20</span></div>
                    <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-full bg-purple-500 rounded-full transition-all" style={{ width: `${(mod.moyenne_quiz / 20) * 100}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
